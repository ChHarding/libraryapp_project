...
class LibApp(tk.Tk):
  ...
  def create_view_library_gui(self, event=None):
    ...
    parent.rowconfigure(0, weight=1)  # Treeview row; allow it to expand vertically
    parent.rowconfigure(1, minsize=50)  # Set a minimum size for the row with buttons
    parent.columnconfigure(0, weight=1)  # Allow the Treeview column to expand
    
    self.tree.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky="ew")

    # Add functionality to edit entries in the treeview
    self.edit_button = tk.Button(parent,
                                 text="Edit",
                                 command=self.edit_selected_book)
    self.edit_button.grid(row=1, column=0, padx=10, pady=(10, 10), sticky="e")
    self.delete_button = tk.Button(parent,
                                   text="Delete",
                                   command=self.delete_selected_book)
    self.delete_button.grid(row=1, column=1, padx=10, pady=(10, 10), sticky="w")
  ...
  def edit_selected_book(self):
    ...
    if len(self.selected_item) == 1:  # Check that exactly one item is selected
  ...
  def update_selected_book(self):
    ...
    row_index = int(self.selected_item[0][1:])# Extract row index from treeview id and adjust for 0-based indexing
